...
* **Data sources.** All analyses are based exclusively on publicly available data retrieved from the following GitHub repositories:
  * **rail-freight-decarbonization**
    https://github.com/LEEYJ1021/rail-freight-decarbonization
  * **korea-freight-rail-resilience-analysis**
    https://github.com/LEEYJ1021/korea-freight-rail-resilience-analysis
* **Data provenance.** For every downloaded file, the analysis automatically records the repository URL, retrieval timestamp (UTC), exact Git commit SHA, SHA-256 checksum, and file size. These provenance records enable complete traceability and should be reported in the manuscript's **Data Availability** statement.
...
* **Cross-validation protocol.** All analyses involving the repeated-measures RQ4 dataset use **leave-one-corridor-out (LOCO) cross-validation**, with clustering performed at the corridor level rather than the individual observation level to avoid information leakage arising from the five scenario-specific observations associated with each station.
* **Randomization and reproducibility.** Random seeds are fixed throughout the analysis (`numpy.random.seed(42)` for the primary pipeline), while robustness analyses document their own independent seeds. All stochastic procedures—including bootstrap confidence intervals, permutation tests, and Monte Carlo sensitivity analyses—report both the number of iterations (e.g., 5,000, 10,000, or 20,000, as appropriate) and the resulting empirical distributions rather than only point estimates.
* **Robustness analyses.** Statistical conclusions are evaluated using complementary robustness procedures, including permutation-based null distributions, nested cross-validation, bootstrap confidence intervals, Holm–Bonferroni correction for multiple comparisons where applicable, scenario-probability sensitivity analysis, and adversarial positive-control testing of the deterministic verification framework.
* **Deterministic verification.** All numerical claims generated by the LLM-assisted explanation layer are verified independently using deterministic Python code rather than another language model. The LLM component is restricted to narrative generation and qualitative assessment, whereas all quantitative validation remains fully reproducible and algorithmic.
* **Computational environment.** The analysis records the Python version, major package versions, Ollama version (where applicable), and the local language models used in the auditing framework to facilitate independent reproduction of both the analytical and LLM-assisted components.
